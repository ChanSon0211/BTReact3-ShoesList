...
import Cart from './Ex3/Cart';
import ProductDetail from './Ex3/ProductDetail';
import ProductItem from './Ex3/ProductItem'
import styles from "./Ex3/ProductList.module.css"
...
export default class ShoesList extends Component {
    ...
    // SetSelected : set biến hứng phù hợp với người dùng truyền
    state = {
         selectedItem: null,
         isShowCart: false,
         cart: [],
            ...
            return (
                ...
                <div key={item.id} className={styles.col}>
                    ...
                    setSelectedItem={this.setSelectedItem}
                     item={item} img={item.image} name={item.name} price={item.price} />
...
decreaseQuantity = (id) => {
    const cloneCart =  [...this.state.cart];
    const foundItem = cloneCart.find((item)=> id === id.prod.id);
    ...
    render() {
        return (
            <div className={styles.container}>
                <h3 onClick={this.showCart} className='text-end pe-5' style={{ cursor: "pointer" , color:"white" }}>Giỏ hàng({this.calcTotalProductInCart()})</h3>
                ...
                {this.state.selectedItem ?(
                    <ProductDetail item={this.state.selectedItem}/>) : null}
